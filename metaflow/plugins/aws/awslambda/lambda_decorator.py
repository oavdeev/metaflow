...
class LambdaDecorator(StepDecorator):
        ...
        for deco in decos:
            ...
            if isinstance(deco, ResourcesDecorator):
                for k, v in deco.attributes.items():
                    if k == "memory":
                        if float(v) > LAMBDA_MAX_MEMORY_MB:
                            raise LambdaException(
                                "{r} cannot exceed {max_v} for *@awslambda* (currently set to {v})".format(
                                    r=k, max_v=LAMBDA_MAX_MEMORY_MB, v=v
                                )
                            )
                        self.memory_mb = v
    ...
    def package_init(
        ...
        if (self.attributes["image"] is None) and (LAMBDA_IMAGE_URI is None):
            raise LambdaException(
                "The *@awslambda* decorator requires either image parameter or "
                "METAFLOW_LAMBDA_IMAGE_URI to be set to a lambda container image uri"
            )
        image_uri = self.attributes["image"] or LAMBDA_IMAGE_URI

        self.lambda_name = lambda_name(
            self.memory_mb, self.run_time_limit, image_uri
        )

        def echo_with_prefix(msg, err):  # type: (str, bool) -> None
            echo(
                "[awslambda decorator] %s"
                % (
                    msg,
                ),
                err=err,
    ...
    def task_pre_step(
        ...
        meta["aws-lambda-log-stream-name"] = os.environ["AWS_LAMBDA_LOG_STREAM_NAME"]
